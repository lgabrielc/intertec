<div class="bg-gray-100">
        ...
        <div class="z-0 bg-white rounded shadow">
            <div class="flex flex-col items-start justify-between w-full p-4 lg:flex-row lg:p-8 lg:items-stretch">
                <div class="flex flex-col items-start w-full lg:w-1/3 lg:flex-row lg:items-center">
                    ...
                    <div class="p-1 mb-2 text-lg border-b">Gestionar Usuarios</div>
                </div>
                                ...
                                <tr class="h-24 border-b border-gray-300">
                                    ...
                                    <td class="pr-6 whitespace-no-wrap">
                                        <div class="flex items-center">
                                            ...
                                            <p class="pt-3 ml-3 text-sm leading-4 tracking-normal text-gray-800 ">
                                                {{ $usuario->nombre }}</p>
                                        </div>
                                    </td>
                                    <td
                                        class="pr-6 text-sm leading-4 tracking-normal text-gray-800 whitespace-no-wrap ">
                                        {{ $usuario->email }}</td>
                                    ...
                                    <td>
                                        ...
                                        <button wire:click="$emit('delete_user',{{ $usuario->id }})"
    ...
    <script>
        Livewire.on('delete_user', this_id_user => {
            const Toast = Swal.mixin({
                ...
                confirmButtonColor: '#3085d6', // Agregar el color del botón de confirmación
                confirmButtonText: 'Sí, eliminar', // Agregar el texto del botón de confirmación
                cancelButtonText: 'Cancelar', // Agregar el texto del botón de confirmación
            })
            Toast.fire({
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    @this.call('delete_user', this_id_user)
                    // Acción a realizar si el usuario confirma la acción
                } else if (result.dismiss === Swal.DismissReason.cancel) {
                    // Acción a realizar si el usuario cancela la acción
                    console.log('El usuario canceló la acción' + this_id_user);
                }
            })
        })
    </script>
